...
class NoisyImageGenerator(Sequence):
    def __init__(self, image_dir_zao, image_dir_yuan, if_n=False, batch_size=32, image_size=64):
        ...
        self.image_zao_paths = sorted(self.image_zao_paths, key=lambda x:x.name[:-6])
    ...
    def __getitem__(self, idx):
        ...
        while True:
            ...
            # image_yuan_path = self.image_yuan_paths[aa//15]
...
class ValGenerator(Sequence):
    def __init__(self, image_dir_zao, image_dir_yuan, if_n=False):
        ...
        image_zao_paths = sorted(image_zao_paths, key=lambda x:x.name[:-6])
        ...
        for i_image in range(self.image_num):
            ...
            # y = cv2.imread(str(image_yuan_paths[i_image//15]))
            if self.if_n:
                x = x / 255.0
                y = y / 255.0
