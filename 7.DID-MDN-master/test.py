...
parser.add_argument('--dataroot', required=False, default='C:/DATASETS/Heavy_rain_image_cvpr2019/val_301_350/pix2pix', help='path to trn dataset')
parser.add_argument('--valDataroot', required=False, default='C:/DATASETS/Heavy_rain_image_cvpr2019/test_with_train_param_v5/pix2pix', help='path to val dataset')
# parser.add_argument('--valDataroot', required=False, default='D:/DATASETS/real_rain/new/pix2pix', help='path to val dataset')
# parser.add_argument('--valDataroot', required=False, default='C:/DATASETS/DID-MDN-datasets/DID-MDN-test', help='path to val dataset')
...
parser.add_argument('--netG', default='./check_50epoch_HR/netG_epoch_39.pth', help="path to netG (to continue training)")
...
if __name__ == '__main__':
    ...
    print(netG)
    num_param_netG = count_parameters(netG)
    print('num of param net G:', num_param_netG)
    netG.train()
    netG.cuda()
    ...
    num_param_net_label = count_parameters(net_label)
    print('num of param net label:', num_param_net_label)
    ...
    num_param_net_res = count_parameters(residue_net)
    print('num of param net residue:', num_param_net_res)
    print('total param of DID-MDN:', (num_param_net_res+num_param_net_label+num_param_netG))
    ...
    for epoch in range(1):
      ...
      for i, data in enumerate(valDataloader, 0):
        if 1:
          ...
          val_input_cpu, val_target_cpu, image_path = data_val
          ...
          from PIL import Image
          # directory = './result_all/new_model_data/'
          directory = './result_test_infer_time/'
          ...
          # filename='./result_all/new_model_data/'+image_name #str(i)+'.png'
          filename= directory+image_name #str(i)+'.png'
          ndarr = tensor.mul(255).clamp(0, 255).byte().permute(1, 2, 0).numpy()
          im = Image.fromarray(ndarr)

          image_size = [512, 512]
          im = im.resize(image_size, resample= Image.BICUBIC)
