...
class SRData(data.Dataset):
    def __init__(self, args, name='', train=True, benchmark=False):
        ...
        if train:
            self.repeat \
                = args.test_every // (len(self.images_lr[0]) // args.batch_size)

    # Below functions as used to prepare images
    def _scan(self):
        names_hr = sorted(
            glob.glob(os.path.join(self.dir_hr, '*' + self.ext[0])), key=lambda x:x[:-3]
        )
        names_lr = [[] for _ in self.scale]
        names_lr[0] = sorted(
            glob.glob(os.path.join(self.dir_lr, '*' + self.ext[1])), key=lambda x:x[:-6]
        )
        # for f in names_hr:
    ...
    def _get_index(self, idx):
        if self.train:
            ...
            return idx % len(self.images_lr[0])
        else:
            return idx

    def _load_file(self, idx):
        idx = self._get_index(idx)
        f_hr = self.images_hr[idx//15]
        # self.images_hr = sorted(self.images_hr, key=lambda x:x[:-3])
        # self.images_lr = sorted(self.images_lr, key=lambda x:x[:-5])
