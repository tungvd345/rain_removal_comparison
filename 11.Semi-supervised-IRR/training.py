...
tf.app.flags.DEFINE_string("rain_name", ".", "The name of data")
...
if __name__ == '__main__':
    ...
    images = tf.compat.v1.placeholder(tf.float32, shape=(None, FLAGS.image_size, FLAGS.image_size, FLAGS.num_channels))  # supervised data (None,64,64,3)
    ...
    g_optim =  tf.compat.v1.train.AdamOptimizer(lr).minimize(loss)
    ...
    with tf.Session(config=config) as sess:
        ...
        for j in range(start_point,epoch):  
            ...
            for num in range(FLAGS.num_h5_file):    # h5 files                
                ...
                psnr = skimage.metrics.peak_signal_noise_ratio(train_label,pred_im)
            ...
            psnr_val = skimage.metrics.peak_signal_noise_ratio(validation_label,pred_val)
